...
# optimized build lives in its own object dir so it never mixes with the debug objects the tests use
OPT_OBJ := build/obj-opt
OPT_OBJECTS := $(patsubst src/%.cpp,$(OPT_OBJ)/%.o,$(wildcard src/*.cpp))
OPT_FLAGS := -O2
...
.PHONY: all jwLLM optimized test clean
...
# make optimized: build with -O2 and run the generator
optimized: build/bin/jwLLM-opt
	./build/bin/jwLLM-opt

build/bin/jwLLM-opt: $(OPT_OBJECTS)
	mkdir -p $(@D)
	$(CXX) $(LDFLAGS) $^ $(LDLIBS) -o $@
...
$(OPT_OBJ)/%.o: src/%.cpp $(JSON_HEADER)
	mkdir -p $(@D)
	$(CXX) $(CPPFLAGS) $(CXXFLAGS) $(OPT_FLAGS) -c $< -o $@
...
# header dependencies generated by -MMD, so editing a .hpp rebuilds every .o that includes it
-include $(wildcard build/obj/*.d) $(wildcard build/obj-opt/*.d)